...
// ─── Cross-reference / interchangeable parts helper ──────────────────────────
// Accepts an array of { brand, articleNo } objects.
// Returns an HTML string grouped by brand, or empty string if no refs.
...
function buildCrossRefsHtml(refs, style = "default") {
  ...
  if (style === "inline") {
    // Compact one-line-per-brand format used by minimal/professional themes
  ...
  if (style === "rows") {
    // Table-row format used by table-focused theme
...
function buildHtmlDefault(data, t) {
  ...
  const xrefsBlock = xrefs.length ? `
  <div style="margin:0 0 12px;">
    <div style="background:${t.primaryColor};color:#ffffff;font-weight:bold;text-align:center;padding:8px 12px;font-size:17px;">Interchangeable Part Numbers:</div>
...
function buildHtmlDarkHeader(data, t) {
  ...
  <div style="margin:0 12px 10px;">
    <div style="background:#111111;color:${t.primaryColor};font-weight:bold;padding:6px 10px;font-size:14px;text-transform:uppercase;letter-spacing:0.8px;">Interchangeable Part Numbers</div>
...
function buildHtmlTableFocused(data, t) {
  ...
  // Compact meta table (OEM + interchangeable + specs side by side)
  const metaRows = [];
  if (oems.length) {
...
<table style="width:100%;border-collapse:collapse;font-family:Arial,sans-serif;border-top:2px solid ${t.primaryColor};">
  ...
  const xrefsBlock = xrefs.length ? `
  <p style="font-weight:bold;font-size:13px;margin:0 0 3px;color:${t.primaryColor};">Interchangeable Part Numbers</p>
...
function buildHtmlProfessionalBlue(data, t) {
  ...
  const xrefsBlock = xrefs.length ? `
    <div style="margin-bottom:12px;">
      <div style="background:${t.primaryColor};color:#ffffff;font-weight:bold;padding:6px 12px;font-size:13px;display:inline-block;margin-bottom:4px;">Interchangeable Part Numbers</div>
