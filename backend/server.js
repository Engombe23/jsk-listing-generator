...
// ─── eBay Sold Listings ────────────────────────────────────────────────────────
// POST /api/ebay/search-prices-sold
// Fetches recently sold/completed UK listings via eBay Finding API.
// Auth: App ID (EBAY_CLIENT_ID) — no OAuth needed for Finding API.
app.post("/api/ebay/search-prices-sold", async (req, res) => {
  try {
    const { query, condition = "new" } = req.body;
    if (!query?.trim()) return res.status(400).json({ error: "query is required" });

    const clientId = process.env.EBAY_CLIENT_ID;
    if (!clientId) throw new Error("eBay credentials not configured (EBAY_CLIENT_ID missing).");

    const condOpt   = conditionOptions.find(c => c.key === condition);
    const condLabel = condOpt?.label || condition;

    // Finding API condition IDs
    const condIdMap = { new: "1000", used: "3000", remanufactured: "2500" };
    const condId    = condIdMap[condition] || null;

    // Re-use same query enrichment as active endpoint
    const rule      = detectProductType(query.trim());
    const ebayQuery = buildEbayQuery(query.trim(), rule);

    // Build Finding API URL — URLSearchParams handles encoding cleanly
    const params = new URLSearchParams({
    ...
    if (condId) {
      ...
      params.set("itemFilter(1).value", condId);
    }
    ...
    if (!ebayRes.ok) {
      const text = await ebayRes.text();
      throw new Error(`eBay Finding API failed (${ebayRes.status}): ${text.slice(0, 300)}`);
    }

    const json       = await ebayRes.json();
    const fbResponse = json?.findCompletedItemsResponse?.[0];
    const ack        = fbResponse?.ack?.[0];

    if (ack !== "Success" && ack !== "Warning") {
      const errMsg = fbResponse?.errorMessage?.[0]?.error?.[0]?.message?.[0] || "Unknown eBay error";
      throw new Error(`eBay Finding API error: ${errMsg}`);
    }

    const rawItems = fbResponse?.searchResult?.[0]?.item || [];

    // Parse Finding API items — all values are wrapped in single-element arrays
    const enriched = rawItems.map(item => {
      // Only include items that actually sold
      if (item.sellingStatus?.[0]?.sellingState?.[0] !== "EndedWithSales") return null;

      const priceStr    = item.sellingStatus?.[0]?.currentPrice?.[0]?.["__value__"];
      const v           = parseFloat(priceStr);
      const shipCostStr = item.shippingInfo?.[0]?.shippingServiceCost?.[0]?.["__value__"];
      const shipV       = parseFloat(shipCostStr);
      const shipType    = item.shippingInfo?.[0]?.shippingType?.[0] || null;
      const fbScore     = parseInt(item.sellerInfo?.[0]?.feedbackScore?.[0], 10);
      const fbPct       = parseFloat(item.sellerInfo?.[0]?.positiveFeedbackPercent?.[0]);
      ...
      return {
        ...
        sellerFeedback:    Number.isFinite(fbScore) ? fbScore : null,
        sellerFeedbackPct: Number.isFinite(fbPct)   ? fbPct   : null,
        shippingCost:      Number.isFinite(shipV) ? shipV : null,
        shippingType:      shipType === "Free" ? "FREE" : shipType,
        itemDate:          item.listingInfo?.[0]?.endTime?.[0] || null,
      };
    }).filter(Boolean);

    const priceValid  = enriched.filter(i => i.price !== null);
    const prices      = priceValid.map(i => i.price).sort((a, b) => a - b);
    const confidence  = getConfidence(prices.length);

    if (prices.length === 0) {
      return res.json({
        low: null, high: null, average: null, median: null,
        currency: "GBP", condition, conditionLabel: condLabel,
        priceCount: 0, listings: [], excludedListings: [],
        confidenceLevel: confidence.level, confidenceLabel: confidence.label, confidenceColor: confidence.color,
      });
    }
    ...
    console.log(`[eBay Sold] "${query}" | ${condLabel} | ${n} sold listings`);
    ...
    console.error("[/api/ebay/search-prices-sold]", err.message);
    res.status(500).json({ error: err.message });
  }
});
